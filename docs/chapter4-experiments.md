...
本章对 **4 种分类算法**进行系统性实验。其中 KNN、SVM、Random Forest 为课堂讲授内容，AdaBoost 为自行查阅文献和官方文档学习的课外算法。
...
| 维度 | KNN | SVM | Random Forest | AdaBoost |
|------|:---:|:---:|:---:|:---:|
| 测试精度 | 91.78% | **93.31%** | 92.36% | 89.84% |
| F1-Score | 0.9179 | **0.9332** | 0.9237 | 0.8982 |
| 过拟合程度 | 2.25% | **0.95%** | 6.41% | 1.72% |
| 推理速度 | 517ms | 610ms | 45ms | **30ms** |
| 训练速度 | **0.001s** | 0.7s | 1.4s | 5.6s |

### 定性对比

| 维度 | KNN | SVM | Random Forest | AdaBoost |
|------|-----|-----|:---:|-----|
| 特征缩放需求 | 必须 | 必须 | 不需要 | 不需要 |
| 可解释性 | 低 | 低 | **高** | 中 |
| 对噪声鲁棒 | 中 | 中高 | **高** | 低 |
| 内存占用 | 高 | 中 | 中 | **低** |
...
**② 速度的两极分化**  
训练阶段：KNN（无训练）vs AdaBoost（5.6s 迭代 50 轮）  
推理阶段：AdaBoost（30ms，50 棵深度 3 树）vs SVM（610ms，大量支持向量）  
部署时需根据延迟要求选择。

**③ 课外算法的教学意义**  
AdaBoost 的精度（89.84%）低于课堂算法，但这恰恰说明了为什么要对比多种算法——**没有任何一种算法在所有数据集上都是最佳选择**。No Free Lunch 定理的实验验证比公式推导更有说服力。
