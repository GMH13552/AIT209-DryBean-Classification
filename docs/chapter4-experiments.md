...
给定待预测样本 x，找出其 K 个最近邻居集合 N_K(x)，预测类别为：
...
<div align="center">
 <img src="images/knn-architecture.svg" alt="KNN架构" style="width:100%;border:1px solid #e2e8f0;border-radius:8px;margin:20px 0" />
</div>
...
现实数据往往不是严格线性可分的。引入松弛变量 ξ_i 和正则化参数 C：
...
$$
 γ 控制每个样本的影响范围： γ 越大，影响范围越小，决策边界越复杂。

#### 多分类扩展

SVM 本身是二分类器。sklearn 默认使用**一对一**（one-vs-one）策略：为每对类别训练一个 SVM，共 
...
<div align="center">
 <img src="images/svm-architecture.svg" alt="SVM架构" style="width:100%;border:1px solid #e2e8f0;border-radius:8px;margin:20px 0" />
</div>
...
<div align="center">
 <img src="images/rf-architecture.svg" alt="RF架构" style="width:100%;border:1px solid #e2e8f0;border-radius:8px;margin:20px 0" />
</div>
...
<div align="center">
 <img src="images/adaboost-architecture.svg" alt="AdaBoost架构" style="width:100%;border:1px solid #e2e8f0;border-radius:8px;margin:20px 0" />
</div>
...
<div align="center">
 <img src="images/gcn-architecture.svg" alt="GCN架构" style="width:100%;border:1px solid #e2e8f0;border-radius:8px;margin:20px 0" />
</div>
...
这是图神经网络最让人困惑的问题。让我们从与 KNN 的对比出发，逐步拆解。

#### KNN 推理 vs GCN 推理（同一个新样本）

假设来了一个新样本，特征为 `[Area=45000, Perimeter=780, ..., Solidity=0.92]`（16 维）。
...
新样本
 → 计算与 9527 个训练样本的距离
 → 找距离最近的 5 个
 → 看这 5 个的标签：[DERMASON, DERMASON, CALI, DERMASON, SEKER]
 → 投票：DERMASON 得 3 票 → 预测 DERMASON
```

KNN 只用了邻居的**标签**。邻居的具体特征值（比如 Area 是 45000 还是 50000）完全不参与预测，只在算距离时用了一次。

**GCN 怎么做？**

```
新样本
 → 找 K=10 个最近邻居（和 KNN 一样）
 → 但这 10 个邻居不直接投票！
 → 而是：每个邻居的 16 维特征 → 乘以 W₁(16×64) → 64 维向量
 → 10 个邻居的 64 维向量加权求和 → 1 个 64 维向量（消息聚合）
 → ReLU 激活 → 保留正数、清零负数
 → 再乘以 W₂(64×7) → 7 维 logits
 → Softmax → 预测
```

#### 参数的作用是什么？

GCN 的 1,543 个参数分为 4 组，各有明确角色：

| 参数 | 形状 | 数量 | 作用 |
|------|------|:---:|------|
| **W₁** | 16 × 64 | 1,024 | 学会"哪些原始特征重要"——比如 Area 的权重是 0.3 还是 -0.1 |
| **b₁** | 64 | 64 | 给每个隐藏维度一个基准值 |
| **W₂** | 64 × 7 | 448 | 学会"哪些隐藏模式对应哪个类别"——比如模式 3 激活时更可能是 DERMASON |
| **b₂** | 7 | 7 | 每个类别的基础倾向（先验概率） |

**具体来说**，用数字举例：

```
Step 1: 邻居 j 的 Area=50000 → W₁[Area, hidden_0] = 0.3
 → 对 hidden_0 的贡献 = 50000 × 0.3 = 15000
 邻居 j 的 Solidity=0.95 → W₁[Solidity, hidden_0] = 0.5
 → 对 hidden_0 的贡献 = 0.95 × 0.5 = 0.475
 ... 16 个特征各贡献一点 → 1 个 64 维隐藏向量 hⱼ

Step 2: 10 个邻居的 h 向量归一化求和 → 1 个 64 维向量 H
 （每个邻居的贡献被度归一化加权，度数高的邻居权重小）

Step 3: ReLU(H) → 负数归零，只保留正向激活
 比如 hidden_0 = -0.2 → ReLU → 0（这个模式不激活）
 hidden_5 = 0.8 → ReLU → 0.8（这个模式激活了！）

Step 4: H × W₂ → 7 个分数
 hidden_5 = 0.8 → W₂[5, DERMASON] = 1.2 → 贡献 +0.96
 hidden_5 = 0.8 → W₂[5, BOMBAY] = -0.3 → 贡献 -0.24
 → DERMASON 分数高，BOMBAY 分数低 → 更可能预测 DERMASON
```
...
GCN：邻居特征 → W₁ 变换 → 聚合 → ReLU → W₂ 变换 → 预测
 └── 有参数，有训练，"学会了怎么看邻居"
```

KNN 像是在问：**"离得近的 5 个邻居是什么豆子？照抄多数。"**

GCN 像是在说：**"我训练了 200 轮，学会了：当邻居的 Area 大、Solidity 高、Eccentricity 低时，大概率是 DERMASON——哪怕它们的标签从没直接告诉我。"**

参数 W₁ 和 W₂ 就是这种"经验"的载体。它们不是人工设定的规则，而是在 9,527 个标注样本上通过梯度下降自动学出来的。
