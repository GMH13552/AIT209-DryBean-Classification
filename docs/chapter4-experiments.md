...
这是图神经网络最容易被忽略的问题。KNN、SVM 等传统模型的推理很直观：输入 16 维特征，输出预测。但 GCN 需要图结构——新样本来时，它和谁连边？
...
新样本 x_test (16 维)
    │
    ▼
① 在训练集+验证集中找 K=10 个最近邻居
   (使用与训练时相同的标准化 + KNN)
    │
    ▼
② 在图中添加新节点，连接它的 10 个邻居
   边方向：邻居 → 新节点 (特征从邻居流向新节点)
    │
    ▼
③ 运行 GCN 前向传播
   Layer 1: 聚合 10 个邻居的特征 → 变换 → ReLU → 64维
   Layer 2: 聚合 10 个邻居的 H1 → 变换 → 7维 logits
    │
    ▼
④ Softmax → 预测类别
```
...
**关键点**：
- 新节点的邻居**全部来自训练集**（已标注样本）
- 新节点本身没有反向影响已训练节点——这是一个**直推式**（transductive）推理
- 图重建的开销很小：只需对 1 个新样本做 KNN 查询（log n 复杂度，kd-tree 加速）

在我们的实现中（`src/gnn.py`），测试阶段确实重建了包含全部训练+测试样本的图：

```python
# 重建图：训练 + 测试全部作为节点
X_all = np.vstack([X_train, X_val, X_test])  # 10874 + 2737 节点
nn = NearestNeighbors(n_neighbors=11)  # K+1，包含自身
nn.fit(X_all)
# 测试节点的邻居是训练节点 → 信息从训练流向测试
edge_test = build_edges(nn.kneighbors(X_all))
# 前向传播
logits = model(X_all, edge_test)
test_pred = logits[test_mask].argmax(dim=1)
```

> 💡 这与 KNN 的推理有本质区别：KNN 只看邻居的**标签**投票；GCN 看邻居的**特征**，经过 2 层非线性变换后才输出预测。GCN 可以学到"哪种特征组合的邻居更有信息量"。
